...
def apply_cba_postprocess(pred_pct: np.ndarray, df: pd.DataFrame = None) -> np.ndarray:
    ...
    NOTE: no buyout override. The app predicts a player's MARKET VALUE on a new
    deal (what they'd command if they signed today), where a one-off buyout from
    a prior season is irrelevant — so we don't special-case buyouts here either.
    Buyouts remain a small, honest source of error on the backward CV (the model
    predicts market value; a below-market buyout signing reads as a miss)."""
