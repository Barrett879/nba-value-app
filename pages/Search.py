...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for name in valid_selected:
        c = careers[name]
        peak_idx = c["Barrett Score"].idxmax()
        peak    = c.loc[peak_idx]
        n_seasons = len(c)
        ...
        rows.append({
            ...
            "Seasons":      n_seasons,
            "Games":        int(c["GP"].sum()),
            "Avg Barrett":  c["Barrett Score"].mean(),
            "Peak Barrett": float(peak["Barrett Score"]),
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "Avg Barrett":  st.column_config.NumberColumn(format="%.2f"),
            "Peak Barrett": st.column_config.NumberColumn(format="%.2f"),
    ...
    st.subheader("Barrett Score — career arcs overlaid")
    ...
    for col, name in zip(cols, valid_selected):
        with col:
            ...
            c = careers[name]
            tbl = c[["Season", "Team", "GP", "PTS", "AST", "REB",
                     "Barrett Score"]].copy()
            peak_season = c.loc[c["Barrett Score"].idxmax(), "Season"]
